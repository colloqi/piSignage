...
function loadBrowser (url) {
    ...
    browser.stdout.on('data', function(data){
        ...
        if (line.indexOf('__executed__') >=0 || line.indexOf('__loaded__') >=0 )
            console.log("uzbl stdout: "+line);

        browserReady = ((previousBrowserCommand.indexOf('uri') == 0)  && (line.indexOf('__loaded__') >=0 )) ||
                            ((previousBrowserCommand.indexOf('uri') != 0)  && (line.indexOf('__executed__') >=0 ));
    })
