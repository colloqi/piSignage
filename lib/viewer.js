...
var browser, currentBrowserUrl,browserReady=true,browserCommand, previousBrowserCommand,
...
function loadBrowser (url) {
    ...
    browser.stdout.on('data', function(data){
        ...
        if (previousBrowserCommand && (line.indexOf('__executed__') >=0 || line.indexOf('__loaded__') >=0 )) {
            if ((previousBrowserCommand.indexOf('uri') == 0)  && (line.indexOf('__loaded__') >=0 ))
                browserReady = true;
            if ((previousBrowserCommand.indexOf('uri') != 0)  && (line.indexOf('__executed__') >=0 ))
                browserReady = true;
            console.log("uzbl stdout: "+line);
            console.log("cmd:"+previousBrowserCommand.toString());
            console.log("browserReady: "+browserReady);
        }
    })
...
function browserSend (cmd) {
    ...
    util.log("Browser command: "+browserCommand)
    if (browser) {
        try {
            browserReady = false;
            previousBrowserCommand = browserCommand;
            setTimeout(function(){browserReady=true;console.log("uzbl: no completion event")},3000);     //after timeout make it true if event does not occur
