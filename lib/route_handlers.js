...
var updateDiskStatus = function () {
    exec('df -h /').stdout.on('data',function(data){
        ...
        diskSpaceUsed = strings[strings.length-2];
        diskSpaceAvailable = Math.floor(strings[strings.length-3]/1000000)+' MB';
    })
}
...
exports.playPlaylist = function (req,res){
    var writeToConfig= function(content){
        fs.writeFile(config.mediaPath+"/"+"_config.json", JSON.stringify(content, null, 4), function(err){
            if (err) throw err;            
        })
    }
    if (req.body.play) {
        ...
        writeToConfig({
            ...
            playingTime: getDiff(playlistStarttime),
    ...
    if (req.body.stop) {
        ...
        currentPlaylist = null;
        writeToConfig({ playingStatus: playlistOn, playingTime: playlistStarttime, playlist: currentPlaylist});
        return;
    }
}
...
exports.getStatus = function(req, res){
    var data= {
        ...
        playingStatus: playlistOn,
        duration: (Date.now() - playlistStarttime),
        lastUpdated:''
    }
