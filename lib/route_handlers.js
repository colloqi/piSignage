    ...
    if(data.image){
        template=
            '<div style="padding:0 40px">'+
             '<div style="float:left;width:60%;padding:10px"><pre style="white-space:pre-wrap">'+
                data.description+'</pre></div>'+
                '<div style="float:right;padding:10px;width:30%">'+
                    '<img src="'+ data.image +'" width=\"100%\" height=\"auto\">'+
                '</div>'+               
            '</div>';
    }else{
        template= '<div style="padding:40px;"><p><pre style="white-space:pre-wrap">'+data.description+'<pre></p></div>';
    }
    var pagedata= '<div style="margin-top: 10%">'+
